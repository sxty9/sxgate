...
indem der Server sich über den **öffentlichen** Hostnamen `ssh.example.com` bei
...
## Vorbedingungen (geprüft 2026-06-13, Host `home`, User `<user>`)
| Check | Status |
|---|---|
| `cloudflared` installiert (2026.6.0) + Service `active` | ✓ |
| Ingress `ssh.example.com → ssh://localhost:22` in `/etc/cloudflared/config.yml` | ✓ |
| DNS `ssh.example.com` → Cloudflare-Edge | ✓ |
...
### 1 — Key-Auth für den Loopback vorbereiten *(User `<user>`, kein sudo)*
...
### 2 — Client-ProxyCommand eintragen *(User `<user>`, kein sudo)*
...
```
**Verify:** `ssh -G ssh.example.com | grep -i proxycommand`
...
## Sicherheitshinweis
Sobald sshd läuft, ist Port 22 über `ssh.example.com` **öffentlich** erreichbar
